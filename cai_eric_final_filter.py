...
with open('cai_eric_final_probs.txt', encoding='unicode-escape') as f:
    word_probabilities = json.load(f)
with open('cai_eric_final_counts.txt') as f:
    count_string = f.readline()
    counts = count_string.split()
    counts = [int(count) for count in counts]

test_file = 'Data/youtube_comments_test.csv'
comments = pd.read_csv(test_file)
...
for comment_text, is_spam in zip(comments['content'], comments['class']):
    actual_spam_ham.append(is_spam)
    subject_spam_prob, subject_spam_log_prob = calculate_spam_probability(comment_text, word_probabilities, counts)
    if subject_spam_prob > threshold:
        predicted_spam_ham.append(1)
    else:
        predicted_spam_ham.append(0)
    if subject_spam_log_prob > threshold:
